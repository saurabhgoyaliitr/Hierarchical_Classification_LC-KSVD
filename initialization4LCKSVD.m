...
% initial points from each classes
...
for classid=1:numClass
    ...
    Dpart = double(Dpart);
   % disp(Dpart);
    l2norms = sqrt(sum(Dpart.*Dpart,1)+eps);
    %disp(repmat(l2norms,size(Dpart,1),1));
    para.initdict = normcols(Dpart);
    %%disp(size(para.initdict));
    
    para.initdict = double(para.initdict);
...
params.initdict = double(params.initdict);
% ksvd process
